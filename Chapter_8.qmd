...
```{r}
lam_counties <- c("Los Angeles", "Orange","San Bernardino", "Riverside")
...
lam_boundaries <- tracts("CA", lam_counties, cb = TRUE, year = 2020) %>% st_transform(3310)

lam_data <- left_join(lam_boundaries,lam_tables, by = "GEOID") %>% select(!c(STATEFP:AFFGEOID, NAME:AWATER)) %>% filter(!GEOID %in% c("06037620101", "06037620001", "06037620002", "06037620102", "06037599100", "06037599000", "06037599100"))
```

#### Checking the median home values

```{r}
lam_map <- ggplot(lam_data, aes(fill = median_valueE)) +
  geom_sf(color = NA) +
  scale_fill_viridis_c(labels = scales::label_dollar()) +
  theme_void() +
  labs(fill = "median home value")

lam_histogram <- ggplot(lam_data, aes(x = median_valueE)) +
  geom_histogram(alpha = 0.5, fill = "navy", color = "navy",
                 bins = 100) +
  theme_minimal() +
  scale_x_continuous(
...
```{r}
lam_map_sqrt <- ggplot(lam_data, aes(fill = sqrt(median_valueE))) + 
  geom_sf(color = NA) + 
  scale_fill_viridis_c() + 
  theme_void() + 
  labs(fill = "Median home\nvalue sqrt")

lam_histogram_sqrt <- ggplot(lam_data, aes(x = sqrt(median_valueE))) + 
  geom_histogram(alpha = 0.5, fill = "navy", color = "navy",
                 bins = 100) + 
  theme_minimal() + 
  scale_x_continuous() + 
...
lam_map_sqrt + lam_histogram_sqrt
```

-   Now we start model fitting

```{r}
lam_data_for_model <- lam_data %>% 
  mutate(
    pop_density = 
      as.numeric(set_units(total_populationE / st_area(.),"1/km2")),
    median_structure_age = 2018 - median_year_builtE) %>% 
...
model1 <- lm(formula = formula, data = lam_data_for_model)
summary(model1)
```
...
```{r}
lam_estimates <- lam_data_for_model %>% 
  select(-GEOID, -median_value, -median_year_built) %>% 
  st_drop_geometry()
correlations <- correlate(dfw_estimates, method = "pearson")
...
model2 <- lm(formula = formula2, data = lam_data_for_model)
summary(model2)
vif(model2)
```
...
```{r}
components <- predict(pca, lam_estimates)

lam_pca <- lam_data_for_model %>% 
  select(GEOID, median_value) %>% 
  cbind(components)

ggplot(lam_pca, aes(fill = PC1)) +
  geom_sf(color = NA) +
  theme_void() +
  scale_fill_viridis_c()
```
...
pca_model <- lm(formula = pca_formula, data = lam_pca)
...
```{r}
lam_data_for_model$residuals <- residuals(model2)

ggplot(lam_data_for_model, aes(x = residuals)) +
  geom_histogram(bins = 100, alpha = 0.5, color = "navy", fill = "navy") +
  theme_minimal()
```
...
```{r}
weights <- lam_data_for_model %>%
  poly2nb(snap = 1000) %>% 
  nb2listw()

moran.test(lam_data_for_model$residuals, weights)
```

-   some mild clustering. let's check the lagged residuals

```{r}
lam_data_for_model$lagged_residuals <- lag.listw(weights,
                                                 lam_data_for_model$residuals)

ggplot(lam_data_for_model, aes(x = residuals, y = lagged_residuals)) +
  theme_minimal() +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "lm", color = "red")
```
...
table(lam_kmeans$cluster)

lam_clusters <- lam_pca %>% 
  mutate(cluster = as.character(lam_kmeans$cluster))



cluster_map <- ggplot(lam_clusters, aes(fill = cluster)) +
  geom_sf(size = 0.01) +
  scale_fill_brewer(palette = "Set1") +
  theme_void() +
  labs(fill = "Cluster")
...
skater_nbrs <- poly2nb(lam_pca, queen = FALSE, snap = 1000)
costs <- nbcosts(skater_nbrs, input_vars)
skater_weights <- nb2listw(skater_nbrs, costs, style = "B")
```
...
```{r}
lam_clusters$region <- as.character(regions$group)

lam_clusters2 <- lam_clusters

cluster_map <- ggplot(lam_clusters2, aes(fill = region)) +
  geom_sf(size = 0.01) +
  scale_fill_brewer(palette = "Set1") +
  theme_void() +
  labs(fill = "region")
