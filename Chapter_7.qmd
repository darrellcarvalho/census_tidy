...
1.  Identify a different core-based statistical area of interest and use the methods introduced in this chapter to extract Census tracts of block groups for that CBSA.
...
2.  Replicate the `erase_water()` cartographic workflow for a different county with significant water area. Be sure to transform your data to an appropriate projected coordinate system first.
...
3.  Acquire a spatial dataset with tidycensus for a region of interest and variable of interest to you. Follow the instructions in this chapter to generate a spatial weights matrix, then compute a hot-spot analysis with `localG()`.
